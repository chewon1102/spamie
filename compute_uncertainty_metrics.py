...
prediction/truth CSVs produced by the uncertainty-weighted-loss
training runs, in the same format as your existing protein_metrics.csv
files (so it plugs directly into your existing stratification/summary
code).

Loops over all 5 graph modes (attention, union, intersection,
spatial, feature) — each trained with use_uncertainty=True,
use_cellfeat=False.
"""
...
def compute_metrics_for_mode(mode):
    result_dir = os.path.join(BASE_RESULTS_DIR, f"human_skin_{mode}_uncertainty", "SpaMIE pred result")
    pred_file = os.path.join(result_dir, f"1human_skin_{mode}_uncertainty_pred.csv")
    true_file = os.path.join(result_dir, f"1human_skin_{mode}_uncertainty_truth.csv")

    output_dir = os.path.join(BASE_RESULTS_DIR, f"human_skin_{mode}_uncertainty", "evaluation")
...
for mode in GRAPH_MODES:
    ...
    all_summaries.append({
        "Graph Mode": f"{mode}_uncertainty",
...
summary_path = os.path.join(BASE_RESULTS_DIR, "graph_mode_uncertainty_summary.csv")
summary_df.to_csv(summary_path, index=False)

print("=== Uncertainty-weighted runs: summary across all 5 modes ===")
print(summary_df)
print(f"\nSaved combined uncertainty summary to {summary_path}")
