import os
import scanpy as sc
import dgl
import random
from tqdm import tqdm
import torch as th
import math
import torch.nn as nn
import torch.nn.functional as F
from sklearn.utils import shuffle
import numpy as np
import pandas as pd

# --- Import from the uncertainty+cellfeat-enabled model file ---
from SpaMIE.spamie_net_uncertainty import *
from SpaMIE.preprocess import preprocessing
from SpaMIE.create_graph import Sagegraph


def set_seed(rndseed, cuda: bool = True, extreme_mode: bool = True):
        os.environ["PYTHONHASHSEED"] = str(rndseed)
        random.seed(rndseed)
        np.random.seed(rndseed)
        th.manual_seed(rndseed)
        if cuda:
            th.cuda.manual_seed(rndseed)
            th.cuda.manual_seed_all(rndseed)
        if extreme_mode:
            th.backends.cudnn.benchmark = False
            th.backends.cudnn.deterministic = True
        dgl.seed(rndseed)
        dgl.random.seed(rndseed)

def sample_mask(idx, l):
    """Create mask."""
    mask = th.zeros(l)
    mask[idx] = 1
    return th.as_tensor(mask, dtype=th.bool)


def adjust_learning_rate(optimizer, epoch, start_lr1=0.02, start_lr2=0.02, lr_decay_rate=0.1, lr_decay_epochs=400, lr_decay_epochs2=400): # simu=0.5
    lr1 = start_lr1 * ( lr_decay_rate ** (epoch // lr_decay_epochs))
    param_group = optimizer.param_groups
    param_group[0]['lr'] =lr1
    param_group[1]['lr'] =lr1
    lr2 = start_lr2 * ( lr_decay_rate ** (epoch // lr_decay_epochs2))
    param_group[2]['lr'] =lr2



class Sagewrapper():

    def __init__(self, seed, device, in_feat, n_hidden, out_feat,task, datatype, sagetype, layers_nums, weight,
                  res_type=None, activation=None, dropout=0.2, epoch=350, lr=1e-4, lr2=0.01, batchnorm=True,
                  # --- cell-feature (bipartite) graph support ---
                  use_cellfeat=False, n_genes_cellfeat=None, cellfeat_layers=2,
                  # --- NEW: optional per-protein uncertainty-weighted loss ---
                  use_uncertainty=False):
        super().__init__()

        self.seeds = seed
        self.device = device
        self.in_feat = in_feat
        self.n_hidden = n_hidden
        self.out_feat = out_feat
        self.activation = activation
        self.dropout = dropout
        self.epoch = epoch
        self.datatype = datatype
        self.lr = lr
        self.lr2 = lr2
        self.layers_nums = layers_nums
        self.res_type = res_type
        self.wt = weight
        self.batchnorm = batchnorm
        self.sagetype = sagetype
        self.task = task
        self.use_cellfeat = use_cellfeat
        self.n_genes_cellfeat = n_genes_cellfeat
        self.cellfeat_layers = cellfeat_layers
        # --- NEW ---
        self.use_uncertainty = use_uncertainty

        if self.task == 'prediction':
            self.model = SpaMIE_pred(self.in_feat, self.n_hidden, self.out_feat, self.wt,
                                           self.activation, self.sagetype, self.layers_nums, self.res_type, self.batchnorm, self.dropout,
                                           use_cellfeat=self.use_cellfeat,
                                           n_genes_cellfeat=self.n_genes_cellfeat,
                                           cellfeat_layers=self.cellfeat_layers,
                                           # --- NEW: forwarded to SpaMIE_pred ---
                                           use_uncertainty=self.use_uncertainty,
                                           ).to(self.device)
        elif self.task == 'integration':
            self.model = SpaMIE_joint(self.in_feat, self.n_hidden, self.out_feat, self.layers_nums, self.dropout, self.wt,
                                            self.activation, self.res_type).to(self.device)
        

    def data_split(self, feat_omics1, train_size):
        y = feat_omics1.shape[0]
        y = th.arange(y)
        sample_number = len(y)
        if train_size is not None:
            sample_idx = np.array(range(len(y)))
            train_idx = sample_idx[:train_size].tolist() 
            val_idx = sample_idx[int(0.7 * y.shape[0]): int(0.75 * y.shape[0])].tolist()
            test_idx = sample_idx[train_size:].tolist()

        else:
            sample_idx = shuffle(np.array(range(len(y))))
            val_idx = sample_idx[int(0.7 * y.shape[0]): int(0.75 * y.shape[0])].tolist()
            train_idx = sample_idx[:int(0.75 * y.shape[0])].tolist()
            test_idx = sample_idx[int(0.75 * y.shape[0]):].tolist()

        train_mask = sample_mask(train_idx, sample_number)
        val_mask = sample_mask(val_idx, sample_number)
        test_mask = sample_mask(test_idx, sample_number)

    
        return train_mask, val_mask, test_mask, train_idx, test_idx


    
    def fit(self, g_spatial_omics1, g_feature_omics1, g_spatial_omics2, g_feature_omics2,
             adata_omics1, adata_omics2, output_dir=None, pred_name=None,
             true_name=None, train_size=None, weight_factors=[1, 5, 1, 1], weight=False, save_csv=False,
             # --- NEW: optional bipartite spot<->gene graph for omics1 ---
             g_cellfeat_omics1=None):
        
        set_seed(self.seeds)

        if self.task == 'prediction':
            # --- NEW: sanity check — fail fast with a clear message rather
            # than a confusing shape error deep inside the model forward pass ---
            if self.use_cellfeat and g_cellfeat_omics1 is None:
                raise ValueError(
                    "Sagewrapper was built with use_cellfeat=True but fit() "
                    "was not given a g_cellfeat_omics1 graph."
                )

            mse = nn.MSELoss()
            feat_omics1 = th.FloatTensor(adata_omics1.obsm['feat'].copy()).to(self.device)

            wt1_param_group = {'params': [self.model.wt1], 'lr': self.lr2}  
            wt2_param_group = {'params': [self.model.wt2], 'lr': self.lr2}  
            excluded_params = [self.model.wt1, self.model.wt2]

            param_groups = [wt1_param_group, wt2_param_group]

            # --- NEW: give log_vars (per-protein uncertainty) its own
            # optimizer group too, same pattern as wt1/wt2 ---
            if self.use_uncertainty:
                log_vars_param_group = {'params': [self.model.log_vars], 'lr': self.lr2}
                param_groups.append(log_vars_param_group)
                excluded_params.append(self.model.log_vars)

            other_params = [param for param in self.model.parameters()
                             if not any(param is ep for ep in excluded_params)]
            param_groups.append({'params': other_params, 'lr': self.lr})

            optimizer = th.optim.AdamW(param_groups)


            train_mask,val_mask,test_mask,train_idx, test_idx = self.data_split(feat_omics1, train_size = train_size) 
            
            
            omics2_X = adata_omics2.X.copy()
            omics2_X = omics2_X
            omics2_X = th.from_numpy(omics2_X).to(self.device)
            omics2_X = omics2_X.float()

            # ------------------------------------------------------------
            # Epoch-loss tracing (carried over from spamie_main_working.py)
            # ------------------------------------------------------------
            epoch_records = []

            vals = []
            for epoch in range(self.epoch):
                self.model.train()
                optimizer.zero_grad()
                if weight:
                    # --- NEW: pass g_cellfeat_omics1 through to the model ---
                    output, wt, alph, latents = self.model(
                        g_spatial_omics1, g_feature_omics1, feat_omics1, weight,
                        g_cellfeat_omics1=g_cellfeat_omics1,
                    )
                else:
                    output, alph = self.model(
                        g_spatial_omics1, g_feature_omics1, feat_omics1, weight,
                        g_cellfeat_omics1=g_cellfeat_omics1,
                    )

                # --- NEW: uncertainty-weighted loss (Kendall et al. 2018) ---
                # per-protein MSE, shape (out_feats,) -- NOT reduced to a
                # scalar yet, since each protein gets its own weight.
                per_protein_mse_train = ((output[train_mask] - omics2_X[train_mask]) ** 2).mean(dim=0)

                if self.use_uncertainty:
                    precision = th.exp(-self.model.log_vars)
                    total_loss = (precision * per_protein_mse_train + self.model.log_vars).sum()
                else:
                    total_loss = mse(output[train_mask], omics2_X[train_mask]).float()

                total_loss.backward()
                optimizer.step()

                # --- Always report PLAIN (unweighted) RMSE in the trace,
                # regardless of what loss was actually optimized, so epoch
                # traces stay directly comparable across experiments ---
                with th.no_grad():
                    plain_train_mse = per_protein_mse_train.mean().item()
                    val_loss = mse(output[val_mask], omics2_X[val_mask]).float()

                epoch_records.append({
                    "epoch": epoch,
                    "train_mse": plain_train_mse,
                    "train_rmse": math.sqrt(plain_train_mse),
                    "val_mse": val_loss.item(),
                    "val_rmse": math.sqrt(val_loss.item()),
                    "optimized_loss": total_loss.item(),  # NEW: the actual objective, for reference
                })

                vals.append(total_loss)
                if min(vals) != min(vals[-20:]):
                    print('Early stopped.')
                    break

            if output_dir is not None:
                epoch_trace_df = pd.DataFrame(epoch_records)
                trace_dir = os.path.join(output_dir, "training_trace")
                os.makedirs(trace_dir, exist_ok=True)
                trace_path = os.path.join(trace_dir, f"{self.seeds}_epoch_loss_trace.csv")
                epoch_trace_df.to_csv(trace_path, index=False)
                print(f"Saved epoch loss trace ({len(epoch_records)} epochs) to {trace_path}")
            # ------------------------------------------------------------

            with th.no_grad():
                self.model.eval()

                if weight:
                    output, wt, alph, latents = self.model(
                        g_spatial_omics1, g_feature_omics1, feat_omics1, weight,
                        g_cellfeat_omics1=g_cellfeat_omics1,
                    )
                    if self.datatype=='Stereo-CITE-seq' or 'simu':
                        output = F.relu(output)
                    train_loss = math.sqrt(mse(output[train_mask], omics2_X[train_mask]))

                else:
                    output, alph = self.model(
                        g_spatial_omics1, g_feature_omics1, feat_omics1, weight,
                        g_cellfeat_omics1=g_cellfeat_omics1,
                    )
                    if self.datatype=='Stereo-CITE-seq' or 'simu':
                        output = F.relu(output)
                    train_loss = math.sqrt(mse(output[train_mask], omics2_X[train_mask]))

                adjust_learning_rate(optimizer, epoch, start_lr1=self.lr2, start_lr2=self.lr)


        

            y_pred = output.cpu()
            y_pred = y_pred.numpy()
            adata_omics2.obsm['predict'] = y_pred
            y_latens = latents.cpu()
            y_latens = y_latens.numpy()
            adata_omics2.obsm['latents'] = y_latens

            if os.path.exists(output_dir+'model'):
                th.save(self.model,output_dir+'model/sage_model.pth')
            else:
                os.makedirs(output_dir+'model')
                th.save(self.model,output_dir+'model/sage_model.pth')

            if save_csv:
                y_pred = y_pred[test_idx]
                omics2_X = omics2_X[test_idx]
                omics2_X = omics2_X.cpu()
                omics2_X = omics2_X.numpy()
                df = pd.DataFrame(y_pred)
                df1 = pd.DataFrame(omics2_X)


                if os.path.exists(output_dir+'SpaMIE pred result'):
                    df.to_csv(output_dir + 'SpaMIE pred result/' + str(self.seeds) + pred_name, index=False)
                    df1.to_csv(output_dir + 'SpaMIE pred result/' + str(self.seeds) + true_name, index=False)
                else:
                    os.makedirs(output_dir+'SpaMIE pred result')
                    df.to_csv(output_dir + 'SpaMIE pred result/' + str(self.seeds) + pred_name, index=False)
                    df1.to_csv(output_dir + 'SpaMIE pred result/' + str(self.seeds) + true_name, index=False)


            if weight:
                return adata_omics1, adata_omics2 , test_idx , train_idx , wt, alph
            else:
                return adata_omics1, adata_omics2 , test_idx , train_idx

        elif self.task == 'integration':
            feat_omics1 = th.FloatTensor(adata_omics1.obsm['feat'].copy()).to(self.device)
            feat_omics2 = th.FloatTensor(adata_omics2.obsm['feat'].copy()).to(self.device)

            mse = nn.MSELoss()
            optimizer = th.optim.AdamW(self.model.parameters(), lr=1e-3)
            weight_factors = weight_factors
            vals = []

            for epoch in range(600):
                self.model.train()
                optimizer.zero_grad()
                output = self.model(g_spatial_omics1, g_feature_omics1, feat_omics1,
                            g_spatial_omics2, g_feature_omics2, feat_omics2)
                loss = (weight_factors[0] * mse(output[2], feat_omics1) +
                        weight_factors[1] * mse(output[3], feat_omics2) +
                        weight_factors[2] * mse(output[4], output[6][-1]) +
                        weight_factors[3] * mse(output[5], output[7][-1]) )
                
                loss.backward()
                optimizer.step()

                vals.append(loss)
                if min(vals) != min(vals[-20:]):
                    print('Early stopped.')
                    break

            return output
