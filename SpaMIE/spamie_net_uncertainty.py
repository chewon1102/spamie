...
class SpaMIE_pred(nn.Module):
    def __init__(self,
        ...
        # --- NEW: one learnable log-variance per protein (output dimension) ---
        # Initialized to 0 -> exp(-0) = 1, so training starts identical to
        # plain unweighted MSE and only diverges as log_vars is learned.
        if self.use_uncertainty:
            self.log_vars = Parameter(th.zeros(out_feats))

        # --- cell-feature encoder + generalized attention (only built if enabled) ---
